...
public:
    ...
    void step() {
        ...
        simulator->project_u();
        swap(simulator->u, simulator->u_);
        simulator->advect_uh();     // advect(u_, u_, 1/2): backtrace u_, interpolate with u_
        simulator->project_uh();
        simulator->advect_u();      // advect(uh, u_, 1/2): backtrace uh, interpolate with u_
        simulator->project_u();
        swap(simulator->u, simulator->u_);
        simulator->advect_d();
        // simulator->diffuse_d();
