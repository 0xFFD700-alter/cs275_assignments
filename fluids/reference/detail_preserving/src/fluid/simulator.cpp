...
advect_uh() {
    ...
    for (int y = 0; y < nh_; ++y) {
        for (int x = 0; x < nw_; ++x) {
            px = (float)x - 0.5f * u_[idxx(x, y)];     // timestep = 1
            py = (float)y - 0.5f * u_[idxy(x, y)];     // timestep = 1
            if (px < 0 && py > y1 && py < y2) {
                uh[idxx(x, y)] = jetv;
                uh[idxy(x, y)] = 0.0f;
                continue;
            ...
            uh[idxx(x, y)] = (1 - iY) * ((1 - iX) * u_[idxx((int)px, (int)py)] + iX * u_[idxx((int)px + 1, (int)py)]) + iY * ((1 - iX) * u_[idxx((int)px, (int)py + 1)] + iX * u_[idxx((int)px + 1, (int)py + 1)]);
            uh[idxy(x, y)] = (1 - iY) * ((1 - iX) * u_[idxy((int)px, (int)py)] + iX * u_[idxy((int)px + 1, (int)py)]) + iY * ((1 - iX) * u_[idxy((int)px, (int)py + 1)] + iX * u_[idxy((int)px + 1, (int)py + 1)]);
        }
    }
}
...
void FluidSimulator::
project_uh() {
    Map uhmap(uh, 2 * nw_ * nh_);
    Map u_map(u_, 2 * nw_ * nh_);
    u_map = uhmap - grad * projSolver.solve(div * uhmap);
    uhmap = 2 * u_map - uhmap;
}
...
advect_u() {
    ...
    for (int y = 0; y < nh_; ++y) {
        for (int x = 0; x < nw_; ++x) {
            px = (float)x - 0.5f * uh[idxx(x, y)];     // timestep = 1
            py = (float)y - 0.5f * uh[idxy(x, y)];     // timestep = 1
            if (px < 0 && py > y1 && py < y2) {
                u[idxx(x, y)] = jetv;
                u[idxy(x, y)] = 0.0f;
                continue;
...
void FluidSimulator::
project_u() {
    Map umap(u, 2 * nw_ * nh_);
    umap = umap - grad * projSolver.solve(div * umap);
}
