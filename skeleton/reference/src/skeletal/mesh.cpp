...
loadFromTinyGLTF(
    ...
    for (size_t geomIndex = 0; geomIndex < meshData.primitives.size(); ++geomIndex) {
        std::cout << "geomIndex: " << geomIndex << std::endl;
    ...
    std::cout << indices.size() << std::endl << positions.size() << std::endl << normals.size()
        << std::endl << influences.size() << std::endl << weights.size() << std::endl;
